...
/** Kart effects (src/fx/, tagged userData.kartFx) are not part of the racer's own build. */
const isKartFx = (o) => { for (let p = o; p; p = p.parent) if (p.userData?.kartFx) return true; return false; };
...
describe('pack B models', () => {
  it.each(IDS)('%s builds a kart-sized, on-budget model like the originals', (id) => {
    ...
    let meshes = 0;
    m.group.traverse((o) => { if (o.isMesh && !isKartFx(o)) meshes++; });
