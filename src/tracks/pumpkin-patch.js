...
export const def = makeTrack(
  ...
  },
  {
    ...
    ops: [
      ...
      { turn: 80, r: 48, y: 2, mark: 'lane-turn' },
      { s: 50, y: 4, mark: 'windmill-lane' },
      { turn: -180, r: 46, y: 5, mark: 'windmill-hairpin' },
      { s: 50, y: 3, mark: 'lane-back' },
      { turn: 90, r: 50, y: 1, mark: 'orchard-turn' },
      { s: 30, y: 0, mark: 'orchard' },
      { turn: 100, r: 60, y: 0, mark: 'home-bend' },
    ],
  },
  (frac, centroid) => ({
    itemBoxRows: [
    ...
    boostPads: [
      // mostly near the centre line: a kid who just follows the road gets the zooms
      { at: frac('start-straight', 'end') - 0.02, lateral: -2 },
      { at: frac('hay-hop', 'end') - 0.006, lateral: 1.5 },
      { at: frac('long-sweeper', 'start') + 0.01, lateral: -1.5 },
      { at: frac('covered-bridge', 'end') + 0.004, lateral: 0 },
      { at: frac('windmill-hairpin', 'end') + 0.012, lateral: 1.5 },
    ],
    scenery: {
      kind: 'pumpkin-patch',
      center: centroid(),
      ...
      windmill: { at: frac('windmill-hairpin', 'mid'), lateral: 46 },
