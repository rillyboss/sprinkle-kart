 ...
 * (same feel as the results screen). Returns { active(), update(dt), handle(ev) -> consumed }.
...
export function celebrationQueue(ctx, host, queue, { delay = 1.2 } = {}) {
  ...
  const show = () => {
    ...
    const u = list.shift();
    if (!u) return;
    current = unlockOverlay(ctx, u, () => handle({ deviceId: 'mouse', action: 'confirm' }));
