...
export function stepKart(kart, input, env, dt) {
  ...
  if (boosting) {
    ...
    if (f > 0.5) f = Math.max(0, f - T.brakeDecel * brake * dt);
  ...
  if (p.hopTime > 0) {
    ...
    p.hopY = Math.sin(Math.PI * (1 - p.hopTime / T.hopDuration)) * T.hopHeight;
  } else {
    p.hopY = 0;
  }
  ...
  if (pressed && !kart.spinning && p.hopTime <= 0 && f > T.driftMinSpeed * 0.8) {
    p.hopTime = T.hopDuration;
    p.driftWindow = T.hopDuration + T.driftStartWindow;
    emit({ type: 'hop', kart });
  }
  if (!kart.drifting && driftBtn && p.driftWindow > 0 && Math.abs(steer) > 0.3 && f > T.driftMinSpeed * 0.8) {
    kart.drifting = true;
  ...
  if (kart.drifting) {
    if (kart.spinning || f < T.driftMinSpeed * 0.6) {
      ...
      p.driftCharge += dt * (0.7 + 0.6 * into);
      let lvl = 0;
      for (let i = 0; i < T.driftLevels.length; i++) if (p.driftCharge >= T.driftLevels[i]) lvl = i + 1;
  ...
  const speedFactor = clamp(absF / T.turnFullSpeed, 0, 1)
    * (1 - T.highSpeedTurnDamp * clamp(absF / (kart.stats.maxSpeed * T.boostMult), 0, 1));
  let yaw;
  if (kart.drifting) {
    const into = (p.steerSmoothed * kart.driftDir + 1) / 2;
    yaw = kart.driftDir * T.turnRate * kart.stats.handling * (T.driftTurnBase + T.driftTurnRange * into);
  } else {
    yaw = p.steerSmoothed * T.turnRate * kart.stats.handling * speedFactor;
  }
  ...
  let f2 = vx * fx + vz * fz;
  let s2 = vx * rx + vz * rz;
  const grip = kart.drifting ? T.driftGrip : kart.spinning ? 3 : T.grip;
  const transfer = kart.drifting ? T.driftGripTransfer : T.gripTransfer;
  const newSide = s2 * Math.exp(-grip * dt);
  if (f2 > 0.5) {
